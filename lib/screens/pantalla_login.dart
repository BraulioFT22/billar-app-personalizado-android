...
class _PantallaLoginState extends State<PantallaLogin> {
  ...
  Future<void> _crearSuperusuario() async {
    ...
    if (nombre.isEmpty) { setState(() => _error = 'Ingresa tu nombre'); return; }
    if (pin.length < 4) { setState(() => _error = 'El PIN debe tener al menos 4 digitos'); return; }
    if (pin != pin2) { setState(() => _error = 'Los PINs no coinciden'); return; }
  ...
  Future<void> _login() async {
    final pin = _pinCtrl.text.trim();
    if (pin.isEmpty) { setState(() => _error = 'Ingresa tu PIN'); return; }
    setState(() => _cargando = true);
    final usuario = await DatabaseService.login(pin);
    if (usuario != null && mounted) {
      SessionService.iniciarSesion(usuario);
      ...
      setState(() { _error = 'PIN incorrecto'; _cargando = false; });
    ...
    return GridView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return ElevatedButton(
          ...
          child: Text(n, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        );
      },
    );
  }
    ...
    return Scaffold(
      body: Center(
        child: SingleChildScrollView(
          child: _necesitaSetup ? _pantallaSetup() : _pantallaLoginNormal(),
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Configuracion inicial', style: TextStyle(color: Colors.grey)),
          const SizedBox(height: 32),
          const Text(
            'Crea tu cuenta de Superusuario.\nEsta cuenta tendra acceso total al sistema.',
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('Selecciona el usuario a resetear:',
                style: TextStyle(color: Colors.grey)),
            const SizedBox(height: 16),
            FutureBuilder<List<Usuario>>(
              ...
              builder: (ctx, snap) {
                ...
                return Column(
                  children: snap.data!.map((u) => ListTile(
                    leading: Icon(
                      u.esSuperusuario ? Icons.star : Icons.person,
                      color: u.esSuperusuario ? Colors.amber : Colors.grey,
                    ),
                    title: Text(u.nombre,
                        style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Text(u.esSuperusuario ? 'Superusuario' : 'Operador',
          ...
          TextButton.icon(
            onPressed: () {
              ...
              Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const PantallaGenerarLicencia()));
            },
          ...
          content: Column(
            ...
            children: [
              TextField(
                ...
                autofocus: true,
                decoration: const InputDecoration(
                  labelText: 'Nuevo PIN (4-6 digitos)',
                  prefixIcon: Icon(Icons.lock),
                  border: OutlineInputBorder(),
                ),
              ),
            ...
            ElevatedButton.icon(
              onPressed: () async {
                if (pinCtrl.text.length < 4) {
                  setDialog(() => error = 'Minimo 4 digitos');
                  return;
                }
                ...
                await DatabaseService.cambiarPin(usuario.id!, pinCtrl.text);
                if (mounted) Navigator.pop(ctx);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('PIN de ${usuario.nombre} actualizado'),
