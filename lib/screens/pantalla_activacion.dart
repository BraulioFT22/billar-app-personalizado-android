...
class _PantallaActivacionState extends State<PantallaActivacion> {
  ...
  @override
  void dispose() {
    _seg1.dispose(); _seg2.dispose(); _seg3.dispose(); _seg4.dispose();
  ...
  Future<void> _cargarDeviceId() async {
    ...
    setState(() { _deviceId = id; _verificando = false; });
  }

  Future<void> _activar() async {
    final codigo =
        'BILLAR-${_seg1.text.toUpperCase()}-${_seg2.text.toUpperCase()}-${_seg3.text.toUpperCase()}-${_seg4.text.toUpperCase()}';
    if (_seg1.text.length < 4 || _seg2.text.length < 4 ||
        _seg3.text.length < 4 || _seg4.text.length < 4) {
      setState(() => _error = 'Ingresa el codigo completo');
      return;
    }
    ...
    if (valido) {
      ...
      setState(() { _error = 'Codigo incorrecto. Contacta al desarrollador.'; _cargando = false; });
      HapticFeedback.mediumImpact();
    }
  }

  Widget _campoSegmento(TextEditingController ctrl, FocusNode foco, FocusNode? siguiente) {
    return SizedBox(
      width: 72,
      child: TextField(
        controller: ctrl,
        ...
        onChanged: (val) {
          if (val.length == 4 && siguiente != null) siguiente.requestFocus();
          setState(() => _error = '');
        },
      ),
    );
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Activacion de Licencia',
                    style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                const Text(
                  'Esta app requiere una licencia valida.\nContacta al desarrollador para obtener tu codigo.',
                  textAlign: TextAlign.center,
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      const Text('ID de tu dispositivo',
                          style: TextStyle(color: Colors.grey, fontSize: 12)),
                      const SizedBox(height: 6),
                      SelectableText(
                        _deviceId,
                        style: const TextStyle(
                            fontSize: 13, fontFamily: 'monospace', color: Colors.white70),
                        textAlign: TextAlign.center,
                      ),
                      const SizedBox(height: 8),
                      TextButton.icon(
                        ...
                        label: const Text('Copiar ID', style: TextStyle(fontSize: 13)),
                      ),
                    ],
                  ),
                ),
                const SizedBox(height: 24),
                const Text('Ingresa tu codigo de activacion:',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                const Text('Formato: BILLAR-XXXX-XXXX-XXXX-XXXX',
                    style: TextStyle(color: Colors.grey, fontSize: 12)),
                const SizedBox(height: 16),
                Row(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(_cargando ? 'Verificando...' : 'Activar Licencia',
